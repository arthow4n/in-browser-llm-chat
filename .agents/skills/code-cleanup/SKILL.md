...
1. **Prefer Explicit Imports**: Many type bypasses or `unknown` usages can be avoided by explicitly exporting and importing the exact types (e.g., `Thread`, `GraphStateType`) from the source files. Default to using the real type definitions whenever possible.
...
4. **XState Testing**: When testing state machine snapshots, instead of `(snapshot.value as any).ViewState`, cast to a minimal structural type like `(snapshot.value as { ViewState?: string }).ViewState`.

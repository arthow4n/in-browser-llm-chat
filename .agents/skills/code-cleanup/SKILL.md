...
3. **Complex Third-Party Generics**: For highly complex library generics (like LangGraph's `StateGraph`), attempt to supply the exact intended generics.
...
5. **No-Mocking Policy**: Remove any `vi.mock()` calls targeting internal modules (e.g. database modules, internal functions, or stores). These are violations of the no-mocking policy. Replace them with real implementations: use `fake-indexeddb` for IndexedDB, and `msw` for network/API layer mocking only. Refactor the test to be an integration-style test using the real module dependency chain.

...
- [ ] **Step 2.1:** Implement the database schema and initialization logic using `idb` inside `src/db/index.ts`. Create stores for `settings`, `presets`, `workflows`, `threads`, `messages`, `checkpoints`, and `checkpoint_writes` exactly as specified in the "Database Schema" section.

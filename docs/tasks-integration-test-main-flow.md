...
- [ ] Ensure `fake-indexeddb` is installed as a dev dependency.
- [ ] Create or update the Vitest setup file (e.g., `src/tests/setup.ts`) to import and initialize `fake-indexeddb` before tests run.
- [ ] Verify that `IndexedDB` is available in the test environment.

...
- System message management CRUD for automatically inserting system message to agents upon API request, but these automatically inserted messages shouldn't be persisted in the chat history.
  - Should support insertion depth (the ability to specify the target index relative to the message history, for example, attaching system message at the Nth message from the beginning or end of the chat messages thread).
...
- **Safety / Cost Control & Loop Controls**:
  ...
  - **Budget Policy and Run Enforcement Details**:
    ...
    - **Temporary Budget Overrides**:
      ...
      - The parent coordinator calculates the temporary override limits using these formulas:
  ...
  - **Force Consensus / Force Summarize early**:
    - **Force Consensus**: Sets the state's `consensusReached` flag to `true` in the execution state and resumes the graph execution, causing the routing logic to bypass further debate rounds and transition straight to the summarizer node.
...
- **Execution & Loop Control Panel (Sticky)**:
  ...
  - **Controls**: Displays the current execution stats. For workflows with loops, it shows the current loop round, turn count, and token usage. For sequential workflows, it shows the current node/step, turn count, and token usage (prompt and completion tokens tracked separately, without currency calculation). Contains buttons to Pause, Resume, or Abort execution, plus "Force Consensus" (updates execution state to set `consensusReached: true`) and "Summarize early" (triggers `FORCE_SUMMARIZE` to bypass remaining loop nodes) buttons specifically visible during loop workflows.
- **Chat Feed**:
...
- **`inactive`**: No background workflow execution is running for the active thread.
  - _Interactive Controls_:
    - Send button: Triggers `SUBMIT_MESSAGE` when clicked. The parent coordinator is responsible for the explicit database write. When `SUBMIT_MESSAGE` is received, the parent coordinator generates a new UUID and sequence number, writes the user's message to the `messages` store in IndexedDB, and appends it to the thread's state to create a checkpoint. Only after this checkpoint is persisted does it transition to `executing` and spawn the runner actor to continue the graph.
...
1. **Generator Initialization**: Upon entering the `initializing` state, the actor instantiates the compiled graph as an `AsyncGenerator` (using the custom runner's stream method or a similar iterator).
2. **Driving the Loop**:

...
- [x] **Step 3.3: Basic 1-Agent Workflow & Message Compilation.** Implement the compiler rules for taking raw message history and forming the exact payload for the LLM API. Hook up the Vercel AI SDK to perform the LLM call using the "Standard 1-agent" approach.
  - [x] Implement the logic/feature.
  - [x] Verify worktree state (`npm run format`, `npm run typecheck`, `npm run lint:fix`, `npm run test`, `npm run build`).
  - [x] Dispatch a subagent to perform code review.
  - [x] Loop and fix any issues found by the code review, tests, typecheck, or linting. Do not proceed until the worktree is completely clean and all issues are resolved.
...
- [x] **Step 4.3: Add Structural Validation for Custom Workflows.** Add JSON schema and graph structure validation checks (e.g., connectivity, loops have exits, exact single entry node, agent-tool wiring) before allowing a workflow to be saved.
  - [x] Implement the logic/feature.
  - [x] Verify worktree state (`npm run format`, `npm run typecheck`, `npm run lint:fix`, `npm run test`, `npm run build`).
  - [x] Dispatch a subagent to perform code review.
  - [x] Loop and fix any issues found by the code review, tests, typecheck, or linting. Do not proceed until the worktree is completely clean and all issues are resolved.
...
- [x] **Step 4.4: Implement Workflow Compilation Factory.** Create the function that converts a validated JSON workflow schema into a runnable/executable custom orchestration graph in the runner. Add dynamic prompt placeholder resolution logic.
  - [x] Implement the logic/feature.
  - [x] Verify worktree state (`npm run format`, `npm run typecheck`, `npm run lint:fix`, `npm run test`, `npm run build`).
  - [x] Dispatch a subagent to perform code review.
  - [x] Loop and fix any issues found by the code review, tests, typecheck, or linting. Do not proceed until the worktree is completely clean and all issues are resolved.
...
- [x] **Step 5.2: Conditional Routing and Consensus Evaluation.** Implement the compiler's conditional routing mechanism (`on_tool_call`, `on_tool_result`, `on_consensus`, `on_no_consensus`). Handle pure-rule vs. LLM-based `consensus_check` nodes.
  - [x] Implement the logic/feature.
  - [x] Verify worktree state (`npm run format`, `npm run typecheck`, `npm run lint:fix`, `npm run test`, `npm run build`).
  - [x] Dispatch a subagent to perform code review.
  - [x] Loop and fix any issues found by the code review, tests, typecheck, or linting. Do not proceed until the worktree is completely clean and all issues are resolved.
...
- [x] **Step 5.4: Implement Multi-Agent Message Rendering.** Refine the message bubble rendering to explicitly denote which agent is speaking (Avatar & Header Bar). Add tool call nesting inside the executing agent's bubble.
  - [x] Implement the logic/feature.
  - [x] Verify worktree state (`npm run format`, `npm run typecheck`, `npm run lint:fix`, `npm run test`, `npm run build`).
  - [x] Dispatch a subagent to perform code review.
  - [x] Loop and fix any issues found by the code review, tests, typecheck, or linting. Do not proceed until the worktree is completely clean and all issues are resolved.
...
- [x] **Step 6.1: Implement `ask_questions` Tool.** Develop the LLM tool definition and the UI rendering for the interactive check-box/comment form in the chat feed (via `ask_questions` Tool Form State Machine). Handle user submission and runner resumption.
  - [x] Implement the logic/feature.
  - [x] Verify worktree state (`npm run format`, `npm run typecheck`, `npm run lint:fix`, `npm run test`, `npm run build`).
  - [x] Dispatch a subagent to perform code review.
  - [x] Loop and fix any issues found by the code review, tests, typecheck, or linting. Do not proceed until the worktree is completely clean and all issues are resolved.
...
- [x] **Step 6.2: Implement Proposal Action Cards.** Create generic approval cards for tools that modify the database (`declare_consensus`, custom workflow creation tools, etc.). Wait for user approval before resuming execution.
  - [x] Implement the logic/feature.
  - [x] Verify worktree state (`npm run format`, `npm run typecheck`, `npm run lint:fix`, `npm run test`, `npm run build`).
  - [x] Dispatch a subagent to perform code review.
  - [x] Loop and fix any issues found by the code review, tests, typecheck, or linting. Do not proceed until the worktree is completely clean and all issues are resolved.
...
- [x] **Step 6.3: Budget Policy Enforcement.** Implement background tracking of autonomous steps (`maxStepsWithoutUser`) and tokens (`maxTokensPerRun`). Fire a `budget_exceeded` interrupt when limits are hit.
  - [x] Implement the logic/feature.
  - [x] Verify worktree state (`npm run format`, `npm run typecheck`, `npm run lint:fix`, `npm run test`, `npm run build`).
  - [x] Dispatch a subagent to perform code review.
  - [x] Loop and fix any issues found by the code review, tests, typecheck, or linting. Do not proceed until the worktree is completely clean and all issues are resolved.
...
- [x] **Step 6.4: Budget Exceeded Card.** Implement the `Budget Exceeded Card` UI component. On user approval, reset the budget counters and resume graph execution.
  - [x] Implement the logic/feature.
  - [x] Verify worktree state (`npm run format`, `npm run typecheck`, `npm run lint:fix`, `npm run test`, `npm run build`).
  - [x] Dispatch a subagent to perform code review.
  - [x] Loop and fix any issues found by the code review, tests, typecheck, or linting. Do not proceed until the worktree is completely clean and all issues are resolved.
...
- [x] **Step 7.3: Thread Branching Logic.** When branching, deep clone the messages array, corresponding checkpoints, and checkpoint writes to a new thread record. Copy the parent's `workflowSnapshot` to ensure consistent execution.
  - [x] Implement the logic/feature.
  - [x] Verify worktree state (`npm run format`, `npm run typecheck`, `npm run lint:fix`, `npm run test`, `npm run build`).
  - [x] Dispatch a subagent to perform code review.
  - [x] Loop and fix any issues found by the code review, tests, typecheck, or linting. Do not proceed until the worktree is completely clean and all issues are resolved.
...
- [x] **Step 7.4: System Message Injection Pipeline.** Refine the compilation rule to dynamically pull global and workflow-specific system messages. Deduplicate and merge them at the specified target indices (`depth`).
  - [x] Implement the logic/feature.
  - [x] Verify worktree state (`npm run format`, `npm run typecheck`, `npm run lint:fix`, `npm run test`, `npm run build`).
  - [x] Dispatch a subagent to perform code review.
  - [x] Loop and fix any issues found by the code review, tests, typecheck, or linting. Do not proceed until the worktree is completely clean and all issues are resolved.
...
- [x] **Step 8.3: Token Stats and Soft/Hard Syncing.** Build the "Sync to Latest Workflow" UI in Thread Settings to perform either a Soft Sync (prompts/presets only) or Hard Sync (destructive snapshot update). Ensure cumulative token statistics recalculate correctly after truncations/edits.
  - [x] Implement the logic/feature.
  - [x] Verify worktree state (`npm run format`, `npm run typecheck`, `npm run lint:fix`, `npm run test`, `npm run build`).
  - [x] Dispatch a subagent to perform code review.
  - [x] Loop and fix any issues found by the code review, tests, typecheck, or linting. Do not proceed until the worktree is completely clean and all issues are resolved.
...
- [x] **Step 9.1: Checkpoint Compaction.** Implement the "Compact Checkpoints" feature within Thread Settings. Hook up the `CheckpointCompactionDialog` UI to dispatch a delete operation for all thread checkpoints and writes EXCEPT the one matching `latestCheckpointId`.
  - [x] Implement the logic/feature.
  - [x] Verify worktree state (`npm run format`, `npm run typecheck`, `npm run lint:fix`, `npm run test`, `npm run build`).
  - [x] Dispatch a subagent to perform code review.
  - [x] Loop and fix any issues found by the code review, tests, typecheck, or linting. Do not proceed until the worktree is completely clean and all issues are resolved.
...
- [x] **Step 9.2: Apply Compaction Effects to UI.** Update all messages whose `checkpointId` was deleted to be `null`. Ensure the UI disables the "Edit", "Delete", and "Branch" options for those messages and displays an explanatory tooltip.
  - [x] Implement the logic/feature.
  - [x] Verify worktree state (`npm run format`, `npm run typecheck`, `npm run lint:fix`, `npm run test`, `npm run build`).
  - [x] Dispatch a subagent to perform code review.
  - [x] Loop and fix any issues found by the code review, tests, typecheck, or linting. Do not proceed until the worktree is completely clean and all issues are resolved.

...
- [x] **Step 2.4:** Fix structural type casting in `src/ui/chat/ChatInputArea.tsx`
  - [x] Define explicit `interface` or `type` definitions and use type guards or Zod schema validation to replace structural cast on Line 38.
  - [x] Verify worktree state (`npm run format`, `npm run typecheck`, `npm run lint:fix`, `npm run test`, `npm run build`).
  - [x] Perform code review (self/subagent).
  - [x] Commit the changes following `AGENTS.md`.
...
- [x] **Step 3.7:** Migrate state in `src/ui/chat/ExecutionControlPanel.tsx`
  - [x] Create an XState machine to manage `isModalOpen` and migrate the component from `useState` to `@xstate/react`.
  - [x] Verify worktree state (`npm run format`, `npm run typecheck`, `npm run lint:fix`, `npm run test`, `npm run build`).
  - [x] Perform code review (self/subagent).
  - [x] Commit the changes following `AGENTS.md`.

- [x] **Step 3.8:** Migrate state in `src/ui/settings/ApiKeyInput.tsx`
  - [x] Create an XState machine to manage `debouncedValue` and migrate the component from `useState` to `@xstate/react`.
  - [x] Verify worktree state (`npm run format`, `npm run typecheck`, `npm run lint:fix`, `npm run test`, `npm run build`).
  - [x] Perform code review (self/subagent).
  - [x] Commit the changes following `AGENTS.md`.

...
- [ ] Implement `truncateMessages(threadId: string, messageId: string)` in `src/db/db.ts` (or a dedicated database service).
- [ ] Verify that messages after the specified ID are correctly deleted from the database.
